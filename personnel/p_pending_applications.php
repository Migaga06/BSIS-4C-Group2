...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $status = $action == 'approve' ? 'Approved by Personnel' : 'Rejected by Personnel';
    $approved_personnel = $action == 'approve' ? 'Yes' : 'No';
    $query = "UPDATE leave_requests SET status='$status', approved_personnel='$approved_personnel', rejection_remarks='$rejection_remarks' WHERE id='$leave_id'";
    if (mysqli_query($conn, $query)) {
        $_SESSION['toast_success'] = "Leave request updated successfully.";
        header('Location: p_pending_applications.php');
        exit();
    } else {
